  ...
  async execute(interaction){
    if (interaction.isModalSubmit()) {
        if(interaction.customId == "BuySystem_Modal") {
            ...
            client17.BcSlashCommands = new Collection();
    ...
    (folder) => !folder.includes(".")
      ...
      f.endsWith(".js")
        ...
        if (command) {
          ...
          client17.BcSlashCommands.set(command.data.name, command);
            ...
            for (let file of readdirSync(folderPath2).filter(f => f.endsWith('.js'))) {
                ...
                client17.on("interactionCreate" , async(interaction) => {
                    if (interaction.isChatInputCommand()) {
                      ...
                      const command = client17.BcSlashCommands.get(interaction.commandName);
                  ...
                  client17.on('ready' , async() => {
                    setInterval(async() => {
                      let systemTokenss = tokens.get(`Bc`)
                  ...
                  await client17.login(Bot_token)
                  if(!system) {
